 ...
 *
 * This class is used to keep the ticket_product taxonomy in sync
 * with EDD Products.
 *
 ...
 */
class EDDSTIX_Product_Sync {

	/**
	 * Name of the post type to use
	 * for populating the ticket_product taxonomy.
	 *
	 * @var    string
	 */
	protected $post_type;
	 ...
	 * Constructor method.
	 *
	 * @param  string  $post_type Name of the post type that should be used to populate the ticket_product taxonomy
	...
	public function __construct( $post_type = '', $taxonomy = '', $append = false ) {
		...
		/* Only hack into the taxonomies functions if multiple products is enabled and the product post type exists */
		...
		if ( $this->is_multiple_products() && post_type_exists( $post_type ) ) {
			 ...
			 * will be synced
			 */
			$sync_init = get_option( "eddstix_sync_$this->post_type" );

			if ( false === $sync_init ) {
				$this->run_initial_sync();
			}

			add_filter( 'get_terms',     array( $this, 'get_terms' ),          1, 4 );
			...
			add_action( 'eddstix_system_tools_table_after', array( $this, 'add_resync_tool' ), 10, 0 );
		}
	 ...
	 * for an argument (at least for our case), we simply ignore it. Called by
	 * get_terms filter.
	 *
	...
	public function map_args( $args ) {
		...
		/* map the taxonomy args to the WP_Query args */
		foreach ( $args as $arg => $value ) {
			...
			switch ( $arg ) {
				...
				case 'include':
					// convert array of term ids ($value) to Product ids
					$download_ids = array();
					if ( ! is_admin() ) {

						if ( is_array( $value ) && $value ) {
							remove_filter( 'get_term', array( $this, 'get_term' ),  1 );
							foreach ($value as $term_id) {
								$term = get_term( $term_id, 'ticket_product' );
								if ( ! is_wp_error( $term ) && isset( $term->name ) ) {
									$download_ids[] = $term->name;
								}
							}
						}
					}
					
					$clean_args['post__in'] = $download_ids ? $download_ids : (array) $value;
					break;
				...
				case 'fields':
					...
					if ( 'count' === $value ) {
						...
						$clean_args['eddstix_get_post_count'] = true; // We set eddstix_get_post_count in order to know that we just need the post count
						continue;
					}
		...
		return apply_filters( 'eddstix_product_sync_mapped_args', $clean_args );
	...
	protected function create_term_object( $post ) {
		...
		$term_data = get_post_meta( $post->ID, '_eddstix_product_term', true );
	...
	public function insert_term( $post ) {
		...
		update_post_meta( $post->ID, '_eddstix_product_term', $term );
	...
	public function protect_insert( $post_id ) {
		set_transient( 'eddstix_product_term_' . $post_id, 1, 5*60 );
	}
	...
	public function unprotect_insert( $post_id ) {
		delete_transient( 'eddstix_product_term_' . $post_id );
	}
	...
	public function is_insert_protected( $post_id ) {
		...
		if ( false === get_transient( 'eddstix_product_term_' . $post_id ) ) {
			return false;
		}
	...
	public function get_terms( $terms, $taxonomies, $args, $term_query ) {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			if ( false === get_option( "eddstix_sync_$this->post_type", false ) ) {
				$this->run_initial_sync();
			}

			if ( isset( $query_args['eddstix_get_post_count'] ) && $query_args['eddstix_get_post_count'] ) {
			...
			foreach ( $query->posts as $key => $post ) {
				...
				// loop through incoming terms to get count of current term.
				foreach ( $terms as $incoming_term ) {

					if ( is_object( $incoming_term ) && is_object( $term ) ) {
						if ( isset( $incoming_term->term_id ) && isset( $term->term_id ) && $incoming_term->term_id == $term->term_id ) {

							// Update the count
							$term->count = $incoming_term->count;
							$new_terms[] = $term;
						}
	...
	public function get_the_terms( $terms, $post_id, $taxonomy ) {
		...
		foreach ( $terms as $key => $term ) {
			...
			if ( $this->is_synced_term( $term->term_id ) ) {
				$download = isset( $term->slug ) ? edd_get_download( $term->slug ) : '';

				/* Set the new values */
				if ( isset( $download->post_title ) ) {
					$terms[ $key ]->name = $download->post_title;
				}

				if ( isset( $download->post_name ) ) {
					$terms[ $key ]->slug = $download->post_name;
				}
				if ( isset( $download->post_content ) ) {
					$terms[ $key ]->description = wp_trim_words( $download->post_content, 55, ' [...]' );
				}
			}
		}
		return $terms;
	...
	public function unsync_term( $post_id ) {
		...
		if ( get_post_type( $post_id ) === $this->post_type ) {
			...
			$term = get_post_meta( $post_id, '_eddstix_product_term', true );

			/* Delete the term */
			$delete = wp_delete_term( (int) $term['term_id'], $this->taxonomy );

			if ( true === $delete ) {
				delete_post_meta( $post_id, '_eddstix_product_term' );
			}
	...
	public function run_initial_sync() {
		...
		add_option( "eddstix_sync_$this->post_type", $count );
		return $count;
	...
	public function add_resync_tool() { ?>
		<tr>
			<td class="row-title"><label for="tablecell"><?php _e( 'Re-Synchronize Products', 'edd-support-tickets' ); ?></label></td>
			<td>
				<a href="<?php echo eddstix_tool_link( 'resync_products', array( 'pt' => $this->post_type ) ); ?>"
				   class="button-secondary"><?php _e( 'Resync', 'edd-support-tickets' ); ?></a>
				<span
					class="eddstix-system-tools-desc"><?php _e( 'Re-synchronize all products from EDD. WARNING: THIS WILL UNASSIGN PRODUCTS FROM EXISTING TICKETS. YOU MAY REASSIGN THEM MANUALLY.', 'edd-support-tickets' ); ?></span>
			</td>
		</tr>
	<?php }
}