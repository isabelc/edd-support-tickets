...
class EDDSTIX_Log_History {
	...
	public function create_log() {
		...
		if ( ! empty( $this->contents ) ) {
			...
			$content .= '<ul class="eddstix-log-list">';
			...
			foreach ( $this->contents as $key => $update ) {
				...
				switch( $action ):
					...
					case 'updated':
						$content .= sprintf( _x( 'updated %s to %s', 'Custom field value was updated', 'edd-support-tickets' ), $label, $value );
						break;

					case 'deleted':
						$content .= sprintf( _x( 'deleted %s', 'Custom field value was deleted', 'edd-support-tickets' ), $label );
						break;

					case 'added':
						$content .= sprintf( _x( 'added %s to %s', 'Custom field value was added', 'edd-support-tickets' ), $value, $label );
						break;
...
function eddstix_log( $post_id = null, $content = '' ) {
	if ( is_null( $post_id ) || empty( $content ) ) {
		return false;
	}

	$log = new EDDSTIX_Log_History( $post_id, $content );
	return $log;
}