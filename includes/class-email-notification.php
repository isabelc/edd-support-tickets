 ...
 *
 * This class handles all email notifications. One instance of this class
 ...
 * available throughout the class and the dispatch of emails is handled by
 * the pluggable function wp_mail(). It is recommended to use a proper SMTP
 * server for email routing in order to ensure a safe delivery.
 *
 ...
 */
class EDDSTIX_Email_Notification {
	...
	public function __construct( $post_id ) {
		...
		if ( ! in_array( get_post_type( $post_id ), array( 'edd_ticket', 'edd_ticket_reply' ) ) ) {
			return new WP_Error( 'incorrect_post_type', __( 'The post ID provided does not match any of the plugin post types', 'edd-support-tickets' ) );
		}
	...
	public function __destruct() {
		...
		/**
		 * Reset the email content type to text as recommended by WordPress
		 *
	...
	public function get_reply() {
		...
		if ( 'edd_ticket_reply' !== get_post_type( $this->post_id ) ) {
			return false;
		}
	...
	public function get_ticket() {
		...
		if ( 'edd_ticket' !== get_post_type( $this->ticket_id ) ) {
			return false;
		}
	 ...
	 *
	 * email notifications can be enabled or disabled on a
	...
	public function is_active( $case ) {
		...
		return boolval( eddstix_get_option( $option, false ) );
	...
	public function get_cases() {
		...
		return apply_filters( 'eddstix_email_notifications_cases', $cases );
	...
	private function cases_active_option() {
		...
		return apply_filters( 'eddstix_email_notifications_cases_active_option', $cases );
	}

	/**
	 * Get sender data.
	 *
	 * @return array Array containing the sender name and email as well as the reply address
	 */
	public function get_sender() {
		...
		$data = array(
			'from_name'   => eddstix_get_option( 'sender_name', get_bloginfo( 'name' ) ),
			'from_email'  => eddstix_get_option( 'sender_email', get_bloginfo( 'admin_email' ) ),
			'reply_email' => eddstix_get_option( 'reply_email', get_bloginfo( 'admin_email' ) ),
		);

		$data['reply_name']  = $data['from_name'];

		$this->data = apply_filters( 'eddstix_email_notifications_sender_data', $data );
	...
	public static function get_tags() {
		...
		$tags = array(
			array(
				'tag' 	=> '{ticket_id}',
				'desc' 	=> __( 'The ticket ID', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{site_name}',
				'desc' 	=> __( 'The website name', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{agent_name}',
				'desc' 	=> __( 'The support agent\'s name', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{agent_email}',
				'desc' 	=> __( 'The support agent\'s email address', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{client_name}',
				'desc' 	=> __( 'The client\'s name', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{client_email}',
				'desc' 	=> __( 'The client\'s email address', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{ticket_title}',
				'desc' 	=> __( 'The ticket title', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{ticket_link}',
				'desc' 	=> __( 'Displays a link to view the ticket on the front-end', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{ticket_url}',
				'desc' 	=> __( 'Displays the URL <strong>only</strong> (not an HTML link) of the front-end ticket', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{ticket_admin_link}',
				'desc' 	=> __( 'Displays a link to ticket details in admin (for agents)', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{ticket_admin_url}',
				'desc' 	=> __( 'Displays the URL <strong>only</strong> (not an HTML link) to ticket details in admin (for agents)', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{date}',
				'desc' 	=> __( 'The current date', 'edd-support-tickets' )
			),
			array(
				'tag' 	=> '{admin_email}',
				'desc' 	=> sprintf( __( 'The WordPress admin email (<em>currently: %s</em>)', 'edd-support-tickets' ), get_bloginfo( 'admin_email' ) )
			),
			array(
				'tag' 	=> '{message}',
				'desc' 	=> __( 'The ticket content or reply content', 'edd-support-tickets' )
			)
		);

		return apply_filters( 'eddstix_email_notifications_template_tags', $tags );
	...
	public function get_tags_values() {
		...
		$agent  = get_user_by( 'id', intval( get_post_meta( $this->ticket_id, '_eddstix_assignee', true ) ) );
		$client = get_user_by( 'id', $this->get_ticket()->post_author );

		/* Get the ticket links */
		$url_public  = get_permalink( $this->get_ticket()->ID );
		$url_private = esc_url( add_query_arg( array( 'post' => $this->ticket_id, 'action' => 'edit' ), admin_url( 'post.php' ) ) );
		...
		foreach ( $tags as $key => $tag ) {
			...
			switch ( $name ) {
				...
				/* Email of the agent assigned to this ticket */
				case 'agent_email':
		...
		$tags = apply_filters( 'eddstix_email_notifications_tags_values', $new, $this->post_id );
	...
	private function get_subject( $case ) {
		return apply_filters( 'eddstix_email_notifications_subject', $this->get_content( 'subject', $case ), $this->post_id );
	}
	...
	private function get_body( $case ) {
		return apply_filters( 'eddstix_email_notifications_body', $this->get_content( 'content', $case ), $this->post_id );
	}
	 ...
	 *
	 * @param  string $part Part of the email to retrieve
	...
	private function get_content( $part, $case ) {
		...
		switch ( $case ) {
			...
			case 'submission_confirmation':
				$value = eddstix_get_option( "{$part}_confirmation", "" );
				break;

			case 'new_ticket_assigned':
				$value = eddstix_get_option( "{$part}_assignment", "" );
				break;

			case 'agent_reply':
				$value = eddstix_get_option( "{$part}_reply_agent", "" );
				break;

			case 'client_reply':
				$value = eddstix_get_option( "{$part}_reply_client", "" );
				break;

			case 'ticket_closed':
				$value = eddstix_get_option( "{$part}_closed", "" );
				break;

		}

		return $this->fetch( apply_filters( 'eddstix_email_notifications_pre_fetch_' . $part, $value, $this->post_id ) );
	 ...
	 *
	 *
	 * @param  string $content_type Current email content type
	 *
	...
	public function notify( $case ) {
		...
		if ( ! $this->notification_exists( $case ) ) {
			return new WP_Error( 'unknown_notification', __( 'The requested notification does not exist', 'edd-support-tickets' ) );
		}

		if ( ! $this->is_active( $case ) ) {
			return new WP_Error( 'disabled_notification', __( 'The requested notification is disabled', 'edd-support-tickets' ) );
		}
		...
		switch ( $case ) {
			...
			case 'new_ticket_assigned':
			case 'client_reply':
				$user = get_user_by( 'id', intval( get_post_meta( $this->ticket_id, '_eddstix_assignee', true ) ) );
				break;
		}
		...
		// add proper line breaks to message body
		$body = $this->text_to_html( $body );
		...
		$headers = array(
			...
			"X-Mailer: EDD Support Tickets/" . EDDSTIX_VERSION,
		);

		/**
		 * Merge all the email variables and apply the eddstix_email_notifications_email filter.
		 */
		$email = apply_filters( 'eddstix_email_notifications_email', array(
...
/**
 * Wrapper function to trigger an email notification.
 *
 * @param  integer $post_id ID of the post to notify about
 ...
 */
function eddstix_email_notify( $post_id, $cases ) {

	$notify = new EDDSTIX_Email_Notification( $post_id );
	$error  = false;
